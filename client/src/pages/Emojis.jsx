...
useEffect(() => {
  const fetchEmojis = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/get_emoji`);
      setEmojis(res.data);
    } catch (error) {
  ...
  fetchEmojis(); // Call the async function
    ...
    const handleAddForm = () =>{
      navigate('/add_emojims')
    }

  return (
    <div className="min-h-screen bg-[#020618] w-full">
     
<div className="p-4 flex flex-row justify-between">
<button
  onClick={handleBack}
  className="bg-slate-800 text-center w-32 rounded-xl h-10 relative text-white text-sm font-semibold group"
  type="button"
>
  <div
    className="bg-sky-400 text-slate-800 rounded-lg h-8 w-1/4 flex items-center justify-center absolute left-1 top-[4px] group-hover:w-[120px] z-10 duration-500"
  >
    <svg
...
style={{
  ...
  className="px-10 py-2 bg-gradient-to-r from-blue-500 to-sky-500 rounded-full shadow-xl group-hover:shadow-2xl group-hover:shadow-blue-600 shadow-blue-600 uppercase font-serif tracking-widest relative overflow-hidden group text-transparent cursor-pointer z-10 after:absolute after:rounded-full after:bg-blue-200 after:h-[85%] after:w-[95%] after:left-1/2 after:top-1/2 after:-translate-x-1/2 after:-translate-y-1/2 hover:saturate-[1.15] active:saturate-[1.4]"
>
  Button
  <p
    className="absolute z-40 font-semibold bg-gradient-to-r from-blue-500 to-sky-500 bg-clip-text text-transparent top-1/2 left-1/2 -translate-x-1/2 group-hover:-translate-y-full h-full w-full transition-all duration-300 -translate-y-[30%] tracking-widest"
  >
   ➕ Add
  </p>
  <p
    className="absolute z-40 top-1/2 left-1/2 bg-gradient-to-r from-blue-700 to-sky-700 bg-clip-text text-transparent -translate-x-1/2 translate-y-full h-full w-full transition-all duration-300 group-hover:-translate-y-[40%] tracking-widest font-extrabold"
  >
    Emojis
  </p>
  <svg
...
<div>
  ...
  <h1 className=" text-4xl text-center font-bold tracking-tight sm:text-6xl lg:text-7xl text-white">
  Express Yourself with <span className="text-sky-400"> Emojis!</span>
      </h1>

     
<div className="relative z-10 flex flex-col items-center justify-start px-4 w-full">
    <div className="w-full max-w-5xl mt-12 px-4 flex justify-center items-center">
    {
      emojis.length > 0 ? <Cards emojis={emojis} /> : <NoResults />
    }
  </div>
  </div>
</div>
